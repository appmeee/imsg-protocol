...
@Test func reactionsForMessageReturnsReactions() throws {
    ...
    try db.run(
        """
        INSERT INTO message(ROWID, handle_id, text, guid, associated_message_guid, associated_message_type, date, is_from_me, service)
        VALUES (5, 2, 'Reacted \u{1F389} to "Hello world"', 'reaction-guid-4', 'p:0/msg-guid-1', 2006, ?, 0, 'iMessage')
        """,
    ...
    #expect(reactions[3].reactionType == .custom("\u{1F389}"))
    #expect(reactions[3].reactionType.emoji == "\u{1F389}")
    #expect(reactions[3].sender == "+456")
}

@Test func reactionsForMessageWithNoReactionsReturnsEmpty() throws {
    let db = try ReactionTestDatabase.makeConnection()
    let now = Date()
...
@Test func reactionsForMessageRemovesReactions() throws {
    let db = try ReactionTestDatabase.makeConnection()
    let now = Date()
    try ReactionTestDatabase.seedBaseMessage(db, now: now)
...
@Test func reactionsForMessageParsesCustomEmojiWithoutEnglishPrefix() throws {
    let db = try ReactionTestDatabase.makeConnection()
    let now = Date()
    try ReactionTestDatabase.seedBaseMessage(db, now: now)
    ...
    try db.run(
        """
        INSERT INTO message(ROWID, handle_id, text, guid, associated_message_guid, associated_message_type, date, is_from_me, service)
        VALUES (2, 2, '\u{1F389} reagiu a "Hello world"', 'reaction-guid-1', 'p:0/msg-guid-1', 2006, ?, 0, 'iMessage')
        """,
        ReactionTestDatabase.appleEpoch(now.addingTimeInterval(-500))
    )
    ...
    #expect(reactions[0].reactionType == .custom("\u{1F389}"))
}

@Test func reactionsMatchGuidWithoutPrefix() throws {
    let db = try ReactionTestDatabase.makeConnection()
    let now = Date()
    try ReactionTestDatabase.seedBaseMessage(db, now: now)
...
@Test func reactionsForMessageRemovesCustomEmojiWithoutEmojiText() throws {
    let db = try ReactionTestDatabase.makeConnection()
    let now = Date()
    try ReactionTestDatabase.seedBaseMessage(db, now: now)
    ...
    try db.run(
        """
        INSERT INTO message(ROWID, handle_id, text, guid, associated_message_guid, associated_message_type, date, is_from_me, service)
        VALUES (2, 2, 'Reacted \u{1F389} to "Hello world"', 'reaction-guid-1', 'p:0/msg-guid-1', 2006, ?, 0, 'iMessage')
        """,
        ReactionTestDatabase.appleEpoch(now.addingTimeInterval(-500))
    )
    try db.run(
...
@Test func reactionsForMessageReturnsEmptyWhenColumnsMissing() throws {
    let db = try Connection(.inMemory)
    try db.execute(
        """
        CREATE TABLE message (
...
@Test func reactionTypeProperties() throws {
    #expect(ReactionType.love.name == "love")
    #expect(ReactionType.love.emoji == "\u{2764}\u{FE0F}")
    #expect(ReactionType.like.name == "like")
    #expect(ReactionType.like.emoji == "\u{1F44D}")
    #expect(ReactionType.dislike.name == "dislike")
    #expect(ReactionType.dislike.emoji == "\u{1F44E}")
    #expect(ReactionType.laugh.name == "laugh")
    #expect(ReactionType.laugh.emoji == "\u{1F602}")
    #expect(ReactionType.emphasis.name == "emphasis")
    #expect(ReactionType.emphasis.emoji == "\u{2757}\u{2757}")
    #expect(ReactionType.question.name == "question")
    #expect(ReactionType.question.emoji == "\u{2753}")
    #expect(ReactionType.custom("\u{1F389}").name == "custom")
    #expect(ReactionType.custom("\u{1F389}").emoji == "\u{1F389}")
}

@Test func reactionTypeFromRawValue() throws {
    ...
    #expect(ReactionType(rawValue: 2006, customEmoji: "\u{1F389}") == .custom("\u{1F389}"))
    #expect(ReactionType(rawValue: 2006) == nil)
    #expect(ReactionType(rawValue: 9999) == nil)
}

@Test func reactionTypeHelpers() throws {
    ...
    #expect(ReactionType.fromRemoval(3006, customEmoji: "\u{1F389}") == .custom("\u{1F389}"))
}
